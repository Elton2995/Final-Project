...
async def create_complaint(
    ...
    notification_crud.create_notification(
        ...
        data={"complaint_id": complaint.id, "complaint_id_display": complaint.complaint_id}
    )
...
async def update_complaint(
    ...
    # Customers can only update open complaints
    if current_user.role == UserRole.CUSTOMER and complaint.status != ComplaintStatus.OPEN:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Cannot update a complaint that is already being processed"
        )
...
async def update_complaint_status(
    ...
    request: Request,
    current_user: UserModel = Depends(require_admin_or_staff),
    db: Session = Depends(get_db)
):
...
async def add_complaint_response(
    ...
    if is_staff:
        ...
        notification_crud.create_notification(
            ...
            data={"complaint_id": complaint.id, "response_id": response.id}
        )
    else:
